...
install_X_S_2026() {
    ...
    while true; do
        ...
        if $ADB_COMMAND devices | grep "$RAW_IP" | grep -q "unauthorized"; then
            echo -e "${YELLOW}👉 Vui lòng nhấn 'Allow' trên TV...${NC}"
            sleep 4
        elif $ADB_COMMAND devices | grep "$RAW_IP" | grep -q "device"; then
            echo -e "${GREEN}✓ Kết nối thành công!${NC}"
            break
        else
            echo -e "→ Đang đợi TV phản hồi..."
            sleep 5
        fi
    done
    ...
    echo "→ Đang thực thi các thiết làm hệ thống..."
